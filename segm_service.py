...
MODEL_YOLO = YOLO('models/model_yolo_final.pt')
MODEL_UNET = load_model('models/model_unet_final.h5')


def segm_yolo(img_path, pixel_spacing):
    class1 = False
    class2 = False
    class3 = False
    results = MODEL_YOLO(img_path, device='cpu')
    img = Image.open(img_path)
    img_cr = Image.open(img_path)
    disk_img = Image.new("L", img.size)
    disk_draw = ImageDraw.Draw(disk_img)
    canal_img = Image.new("L", img.size)
    canal_draw = ImageDraw.Draw(canal_img)
    spine_img = Image.new("L", img.size)
    spine_draw = ImageDraw.Draw(spine_img)
    other_img = Image.new("L", img.size)
    ...
    for r in results:
        ...
        for _id, label in enumerate(labels):
            try:
                if clss[_id] == 0.:
                    disk_draw.polygon(label, fill=255)
                    class1 = True
                elif clss[_id] == 1:
                    canal_draw.polygon(label, fill=255)
                    class2 = True
                elif clss[_id] == 2:
                    spine_draw.polygon(label, fill=255)
                    class3 = True
                elif clss[_id] == 3:
                    other_draw.polygon(label, fill=255)
            except TypeError:
                pass

    background_arr = np.zeros((320, 320, 4), dtype=np.uint8)
    background = Image.fromarray(background_arr)
    mask1 = get_mask(np.array(disk_img)[:, :], (0, 204, 0, 80))
    img.paste(mask1, (0, 0), mask1)
    background.paste(mask1, (0, 0), mask1)
    mask2 = get_mask(np.array(canal_img)[:, :], (204, 0, 204, 100))
    img.paste(mask2, (0, 0), mask2)
    background.paste(mask2, (0, 0), mask2)
    mask3 = get_mask(np.array(spine_img)[:, :], (0, 0, 255, 120))
    img.paste(mask3, (0, 0), mask3)
    background.paste(mask3, (0, 0), mask3)
    mask4 = get_mask(np.array(other_img)[:, :], (0, 255, 255, 100))
    img.paste(mask4, (0, 0), mask4)
    background.paste(mask4, (0, 0), mask4)

    bbox = background.getbbox()
    img_cr = img_cr.crop((bbox[0] - 10, bbox[1] - 10, bbox[2] + 10, bbox[3] + 10))
    img_cr = img_cr.resize((320, 320), Image.Resampling.LANCZOS)

    file_title_crop = str(uuid.uuid4())
    file_name_crop = 'output/' + file_title_crop + '.png'
    img_cr.save(file_name_crop)
    ...
    image_segm = Image.fromarray(image)
    ...
    return {"is_segment": class1 == True and class2 == True and class3 == True,
            "disk_area": disk_area,
